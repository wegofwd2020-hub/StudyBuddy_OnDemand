...
| 0072 | #762 — `question_validations`, per-school "this answer was checked" state. PK `(school_id, stable_question_id)`, RLS `tenant_isolation` copied from 0059. Keyed by the migration-0067 `stable_question_id` rather than a position because `q1` names a SLOT within a set, not a question, and the same question appears in several sets. `correct_text` is a SNAPSHOT of the correct option's normalised text at validation time: when the current correct answer no longer matches it the page says "needs re-checking" instead of showing a stale tick. Not stored in 0069's `quiz_questions` registry — that table is platform-wide with no writer in the app, and validation is per school |
