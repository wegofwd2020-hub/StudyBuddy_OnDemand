...
| 0072 | One email, one role — cross-table trigger on `teachers`/`students` rejecting an address held by the other. Each table's `email` has been UNIQUE since 0001; the missing half was cross-table, so a person could exist as both and `login_local_user` (teachers first, students on fallthrough) made the student row UNREACHABLE — the wrong portal, no error. A trigger, not an app check: 12 code paths insert these rows and a provisioning-only guard would leave the rest opted out. Scoped to `auth_provider='local'` (the only track the email lookup uses — a broader rule blocked the legitimate register-a-school-then-try-the-demo flow) and it ignores `account_status='deleted'` rows, since deletion is soft under ADR-005 and must not reserve an address forever. Raises as `unique_violation` with `CONSTRAINT='one_email_one_role'` so callers that already branch on `constraint_name` need no new error shape. Leaves a narrow concurrent-insert race; the airtight fix is a shared identity table, filed as follow-up |
