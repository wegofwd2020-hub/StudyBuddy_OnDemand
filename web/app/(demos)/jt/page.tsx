      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mt-6 rounded-xl border border-dashed border-gray-200 bg-white px-5 py-3 text-xs text-gray-500">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
            ...
            <span>
              ~{SCENARIO_LIST.reduce((acc, s) => acc + s.metrics.authoring_time_min, 0)}{" "}
              min total build
            </span>
            <span aria-hidden className="text-gray-300">
              ·
            ...
            <span>
              ~$
              {SCENARIO_LIST.reduce((acc, s) => acc + totalCost(s.metrics), 0).toFixed(
                2,
              )}{" "}
              total cost
            </span>
          ...
          <p className="mt-1 text-[11px] text-gray-400">
            Costs are estimates. LLM = broker (Claude Opus 4.7) authoring tokens. img =
            image-generation API for stylized portraits (0 if default photo used). D-ID =
            `/talks` API credits for talking-avatar video. &ldquo;Preview&rdquo; scenarios
            have no D-ID cost yet because videos haven&apos;t been generated.
          </p>
        </div>
