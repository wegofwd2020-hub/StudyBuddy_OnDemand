...
import {
  ...
  extendDemoTeacherAccount,
  revokeDemoTeacherAccount,
  adminResendDemoTeacherVerification,
  type DemoTeacherAccountItem,
} from "@/lib/api/admin";
...
const STATUS_META: Record<
  DemoTeacherAccountItem["request_status"],
...
function StatusBadge({ status }: { status: DemoTeacherAccountItem["request_status"] }) {
...
function ExtendModal({
  ...
  async function handleExtend() {
    ...
    setSubmitting(true);
    try {
      await extendDemoTeacherAccount(item.account_id, hours);
      toast.success(`Extended teacher demo for ${item.email} by ${hours}h`);
      onDone();
    } catch {
      toast.error("Failed to extend teacher demo account");
      setSubmitting(false);
    }
  }
      ...
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={submitting}
            className="flex-1 rounded-lg bg-cyan-600 py-2 text-sm font-medium text-white transition-colors hover:bg-cyan-700 disabled:opacity-50"
          >
...
function RevokeModal({
  ...
  async function handleRevoke() {
    ...
    setSubmitting(true);
    try {
      await revokeDemoTeacherAccount(item.account_id);
      toast.success(`Revoked teacher demo for ${item.email}`);
      onDone();
    } catch {
      toast.error("Failed to revoke teacher demo account");
      setSubmitting(false);
    }
  }
...
function RowActions({
  ...
  async function handleResend() {
    setResending(true);
    try {
      await adminResendDemoTeacherVerification(item.request_id);
      ...
      <div className="flex items-center gap-1.5">
        {item.verification_pending && (
          <button
            ...
            className="flex items-center gap-1 rounded-md bg-cyan-50 px-2 py-1 text-xs font-medium text-cyan-600 transition-colors hover:bg-cyan-100 disabled:opacity-50"
          >
        ...
        {item.account_id && item.request_status === "verified" && (
          <>
            <button
              ...
              title="Extend teacher demo access"
              className="flex items-center gap-1 rounded-md bg-cyan-50 px-2 py-1 text-xs font-medium text-cyan-600 transition-colors hover:bg-cyan-100"
            >
            ...
            <button
              ...
              title="Revoke teacher demo access"
...
export default function AdminDemoTeacherAccountsPage() {
  ...
  function refresh() {
    qc.invalidateQueries({ queryKey: ["admin", "demo-teacher-accounts"] });
  }
  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      <h1 className="mb-1 text-2xl font-bold text-gray-900">Teacher Demo Accounts</h1>
      <p className="mb-6 text-sm text-gray-500">
        Manage teacher demo requests and active 48-hour teacher demo sessions.
      </p>
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center text-gray-400">
          ...
          <p className="text-sm">No teacher demo accounts found.</p>
        </div>
      )}
    </div>
  );
