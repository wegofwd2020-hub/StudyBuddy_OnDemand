...
export function ScenarioPlayer({ scenario, autoPlayDelayMs = 2500 }: ScenarioPlayerProps) {
  ...
  const questions: QuizQuestion[] = (() => {
    if (scenario.quiz_questions && scenario.quiz_questions.length > 0) return scenario.quiz_questions;
    if (scenario.quiz) {
      return [{
        id: "q0",
        ...
        {scenario.characters.map((c) => {
          ...
          return (
            <div key={c.id} className="flex items-center gap-2">
              <div className={cn("flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white", color)}>
                {initials(c.name)}
              </div>
              <div>
        ...
        {scenario.dialog.slice(0, visibleCount).map((turn, i) => {
          ...
          return (
            <div key={i} className={cn(
              ...
              isLeft ? "justify-start slide-in-from-left-4" : "flex-row-reverse justify-start slide-in-from-right-4",
            )}>
              <div className={cn("flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white", color)}>
              ...
              <div className={cn(
                ...
                isLeft ? "rounded-tl-sm border border-gray-200 bg-white text-gray-800" : "rounded-tr-sm bg-indigo-600 text-white",
              )}>
                <p className={cn("mb-1 text-xs font-semibold", isLeft ? "text-blue-600" : "text-indigo-200")}>
...
function QuizInput({ question, onAnswer }: { question: QuizQuestion; onAnswer: (a: boolean | string | string[]) => void }) {
        ...
        {(question.options ?? []).map((opt) => (
          <label key={opt.id} className={cn(
            ...
            <input type="checkbox" checked={selected.includes(opt.id)}
              onChange={() => setSelected((p) => p.includes(opt.id) ? p.filter((x) => x !== opt.id) : [...p, opt.id])}
...
function QuizResult({ question, answer, hasNext, onNext, onReplay }: {
  ...
  function label(val: boolean | string | string[]): string {
    ...
    if (question.format === "single_choice") return question.options?.find((o) => o.id === val)?.text ?? String(val);
    return (val as string[]).map((id) => question.options?.find((o) => o.id === id)?.text ?? id).join(", ");
  }
    ...
    <div className="space-y-4">
      <div className={cn("flex items-start gap-3 rounded-lg px-4 py-3",
        ...
        <p className="font-semibold">
          {correct ? "Correct!" : `Incorrect. Correct answer: ${label(question.correct_answer)}.`}
        </p>
      </div>
...
function ScoreSummary({ questions, answers, onReplay }: {
