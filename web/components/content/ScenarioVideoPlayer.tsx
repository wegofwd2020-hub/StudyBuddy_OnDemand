...
function VideoDialogPlayer({
  ...
  const questions: QuizQuestion[] = (() => {
    const qs = (scenario as unknown as { quiz_questions?: QuizQuestion[] })
      .quiz_questions;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      {phase === "idle" && (
        <div className="flex flex-col items-center justify-center gap-4 rounded-xl border bg-gray-900 py-16 shadow-lg">
          <p className="text-sm text-gray-400">
            Watch the scenario unfold, then answer the compliance question
      ...
      {phase === "done" && (
        <div className="space-y-3 rounded-xl border-2 border-indigo-200 bg-indigo-50 p-6 text-center shadow-md">
      ...
      <div className="space-y-2">
        {(question.options ?? []).map((opt) => (
          <button
            ...
            className="w-full rounded-lg border-2 border-indigo-200 bg-white px-4 py-2.5 text-left text-sm font-medium text-gray-800 transition-colors hover:border-indigo-400 hover:bg-indigo-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
          >
            {opt.text}
          </button>
        ))}
...
function QuizResult({
  ...
  function correctLabel(): string {
    if (question.format === "true_false")
      return String(question.correct_answer).toUpperCase();
    if (question.format === "single_choice") {
      return (
      ...
      >
        ...
        <p className="font-semibold">
          ...
          {!correct && (
            <>
              {" "}
              You answered: <em>{answerLabel()}</em>. Correct: <em>{correctLabel()}</em>.
            </>
          )}
        </p>
      </div>
