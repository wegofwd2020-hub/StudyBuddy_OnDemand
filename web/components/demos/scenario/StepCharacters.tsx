...
export function StepCharacters({ draft, onChange }: Props) {
  ...
  function addCharacter() {
    ...
    next[next.length - 1].voice_id = voices[Math.min(next.length - 1, voices.length - 1)].id;
  ...
  return (
    <div className="space-y-5">
      ...
      {draft.characters.map((char, idx) => (
        <div key={char.id} className="rounded-xl border bg-white p-5 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Approx. Age</label>
              <input
                type="number"
                min={18}
                max={75}
                value={char.approx_age ?? ""}
                onChange={(e) => updateCharacter(idx, { approx_age: Number(e.target.value) || undefined })}
                placeholder="e.g. 38"
                className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200"
              />
            </div>
            <div>
              ...
              <select
                value={char.gender}
                onChange={(e) => updateCharacter(idx, { gender: e.target.value as ScenarioCharacter["gender"] })}
                className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 bg-white"
              >
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Ethnicity (optional)</label>
              <input
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Animation Style</label>
              <select
                value={char.animation_style}
                onChange={(e) => updateCharacter(idx, { animation_style: e.target.value as "realistic" | "animated" })}
                className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 bg-white"
              >
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Background / Setting</label>
              <select
