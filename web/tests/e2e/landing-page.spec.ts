...
test("PUB-07 — bottom CTA 'Start your free trial' navigates to /signup", async ({
  page,
}) => {
  await page.goto("/");
...
for (const { text, href } of NAV_LINKS) {
  test(`PUB-08/09 — desktop nav link "${text}" navigates to ${href}`, async ({
    page,
  }) => {
    await page.goto("/");
...
test("PUB-10 — hamburger menu is visible and opens mobile nav at 375 px viewport", async ({
  ...
  const desktopPricingLink = page.locator("nav.hidden.md\\:flex a", {
    hasText: "Pricing",
  });
